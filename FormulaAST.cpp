...
namespace ASTImpl {
	...
	public:
		...
		virtual double Evaluate(ExecuteCell& execute_cell) const = 0;

		// higher is tighter
		virtual ExprPrecedence GetPrecedence() const = 0;

		void PrintFormula(ostream& out, ExprPrecedence parent_precedence, bool right_child = false) const {
	...
	namespace {
		...
		public:
			...
			double Evaluate(ExecuteCell& execute_cell) const override {
				auto lhs_value = lhs_->Evaluate(execute_cell);
				auto rhs_value = rhs_->Evaluate(execute_cell);
				double result;
		...
		public:
			...
			double Evaluate(ExecuteCell& execute_cell) const override {
				if (type_ == UnaryPlus) {
					return +operand_->Evaluate(execute_cell);
				} else {
					return -operand_->Evaluate(execute_cell);
				}
			}
		...
		public:
			...
			double Evaluate(ExecuteCell&) const override {
				return value_;
			}
...
double FormulaAST::Execute(ExecuteCell& execute_cell) const {
	return root_expr_->Evaluate(execute_cell);
}
