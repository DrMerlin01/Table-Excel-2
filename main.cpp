...
inline ostream& operator<<(ostream& output, Position pos) {
...
inline ostream& operator<<(ostream& output, Size size) {
	return output << "(" << size.rows << ", " << size.cols << ")";
}

inline ostream& operator<<(ostream& output, const CellInterface::Value& value) {
	visit(
		[&](const auto& x) {
			output << x;
		},
...
namespace {
	/*string ToString(FormulaError::Category category) {
		return string(FormulaError(category).ToString());
	}*/

	void TestPositionAndStringConversion() {
		auto testSingle = [](Position pos, string_view str) {
			ASSERT_EQUAL(pos.ToString(), str);
			ASSERT_EQUAL(Position::FromString(str), pos);
		};

		for (int i = 0; i < 25; ++i) {
			testSingle(Position{i, i}, char('A' + i) + to_string(i + 1));
		}
	...
	void TestSetCellPlainText() {
		...
		ASSERT_EQUAL(get<string>(cell->GetValue()), "=escaped");
	}
	...
	void TestFormulaArithmetic() {
		auto sheet = CreateSheet();
		auto evaluate = [&](string expr) {
			return get<double>(ParseFormula(move(expr))->Evaluate(*sheet));
		};
	...
	void TestFormulaExpressionFormatting() {
		auto reformat = [](string expr) {
			return ParseFormula(move(expr))->GetExpression();
		};
	...
	void TestFormulaReferencedCells() {
		...
		ASSERT_EQUAL(a1->GetReferencedCells(), (vector{"A1"_pos}));

		auto b2c3 = ParseFormula("B2+C3");
		ASSERT_EQUAL(b2c3->GetReferencedCells(), (vector{"B2"_pos, "C3"_pos}));

		auto tricky = ParseFormula("A1 + A2 + A1 + A3 + A1 + A2 + A1");
		ASSERT_EQUAL(tricky->GetExpression(), "A1+A2+A1+A3+A1+A2+A1");
		ASSERT_EQUAL(tricky->GetReferencedCells(), (vector{"A1"_pos, "A2"_pos, "A3"_pos}));
	}
	...
	void TestFormulaInvalidPosition() {
		auto sheet = CreateSheet();
		auto try_formula = [&](const string& formula) {
			try {
	...
	void TestCellReferences() {
		...
		ASSERT_EQUAL(sheet->GetCell("A2"_pos)->GetReferencedCells(), vector{"A1"_pos});
		ASSERT_EQUAL(sheet->GetCell("B2"_pos)->GetReferencedCells(), vector{"A1"_pos});

		// Ссылка на пустую ячейку
		sheet->SetCell("B2"_pos, "=B1");
		ASSERT(sheet->GetCell("B1"_pos)->GetReferencedCells().empty());
		ASSERT_EQUAL(sheet->GetCell("B2"_pos)->GetReferencedCells(), vector{"B1"_pos});
		...
		ASSERT_EQUAL(sheet->GetCell("B1"_pos)->GetReferencedCells(), vector{"C3"_pos});
	}

	void TestFormulaIncorrect() {
		auto isIncorrect = [](string expression) {
			try {
				ParseFormula(move(expression));
			} catch (const FormulaException&) {
				return true;
			}
			return false;
